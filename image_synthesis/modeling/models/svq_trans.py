...
from image_synthesis.modeling.modules.edge_connect import losses
...
#         act = nn.GELU() if activate == 'GELU' else nn.ReLU
#         if mlp_type == 'linear':
...
class MaskedImageInpaintingTransformer(nn.Module):
    def __init__(
            ...
            *,
            ...
            content_codec_config=None,  # image_synthesis.modeling.codecs.image_codec.svq_gan.PatchVQGAN
        ...
        out_cls = self.content_codec.get_number_of_tokens() if num_token is None else num_token    # n_e
    ...
    def sample_largest(
        ...
        while content_mask.sum() < content_mask.numel():
            ...
            if num_token_per_iter == -1 or num_token_per_iter >= self.content_seq_len:
                filtered_logits = logits_top_k(logits, filter_ratio=filter_ratio, minimum=1,
                                               filter_type=filter_type)  # b x HW x C
                ...
                for bidx in range(batch_idx.shape[0]):
                    ...
                    for bidx_j in range(batch_idx.shape[1]):
                        if batch_idx[bidx, bidx_j]:
                            att_weight[bidx][int(index[bidx, bidx_j])] = att_w[bidx][int(index[bidx, bidx_j])].view(
                                tuple(self.content_codec.token_shape)).to('cpu')  # HW -> H x W

            # sample
            filtered_logits = logits_top_k(logits, filter_ratio=filter_ratio, minimum=1,
                                           filter_type=filter_type)  # b x num x C
            probs = F.softmax(filtered_logits * temperature, dim=-1)  # b x num x C
    ...
    def sample(  # sample_raster
        ...
        for i in bar:
            ...
            if pred:
                ...
                # sample
                filtered_logits = logits_top_k(logits, filter_ratio=filter_ratio, minimum=1,
                                               filter_type=filter_type)  # B x 1 x C
                ...
                # import pdb; pdb.set_trace()
                content_feat[:, start_:end_] = sample_feat * (
                            1 - itr_cont_mask.to(sample_feat.dtype).unsqueeze(dim=-1)) + content_feat[:,
                                                                                         start_:end_] * itr_cont_mask.to(
                    sample_feat.dtype).unsqueeze(dim=-1)
                content_token[:, start_:end_][~itr_cont_mask] = sample[~itr_cont_mask]
                content_mask[:, start_:end_] = True
                assert content_mask[:,
                       :end_].sum() == b * end_  # make sure that previous content tokens are all unmasked
    ...
    def prepare_data(self, image, mask):
        ...
        if self.input_feature_type == 'origin':
            ...
            if self.random_quantize > 0:
                quantize_mask = torch.rand(*token_type.shape).to(
                    token_type.device) < self.random_quantize  # B x 1 x H x W, in range [0, 1)
                ...
                feat_mask = feat_mask * (1 - quantize_mask) + (
                            data_mask['feature_quantize'] * quantize_mask)  # B x C x H x W
        elif self.input_feature_type == 'quantized':
            feat_mask = data_mask['feature_quantize']  # B x C x H x W
            b, _, h, w = feat_mask.shape
            token_type = get_token_type(mask, type='pixel_shuffle', token_shape=[h, w])  # B x 1 x H x W
        ...
        token_target = self.content_codec.get_features(image, return_token=True)['token'].view(b,
                                                                                               h * w).contiguous()  # B x HW
        # import pdb; pdb.set_trace()
        feat_mask = self.emb_proj(feat_mask.permute(0, 2, 3, 1).view(b, h * w, -1).contiguous())  # B x HW x D
    ...
    def forward(
        ...
        if return_loss:
            ...
            # import pdb; pdb.set_trace()
            loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), token_target.view(-1),
                                   ignore_index=self.content_ignore_token, reduction='none')
            # import pdb; pdb.set_trace()
...
class SemanticAwareTransformer(nn.Module):
    def __init__(self, n_layer=6,
                 n_embed=512,
                 n_head=8,
                 content_seq_len=1024,  # hxw
                 ...
                 weight_decay=0.01):
        super(SemanticAwareTransformer, self).__init__()
        ...
        if name is None or name == 'none':
            ...
            if name == 'generator':
                params = list(self.encoder.parameters()) + \
                         ...
                         list(self.content_codec.parameters())
    ...
    @property
    def device(self):
        return self.content_codec.device
    ...
    def forward(self,
        ...
        if name == 'generator':
            img = self.pre_process(batch['image'])
            mask = batch['mask']
            x = self.multi_pixels_with_mask(img, mask)
            x = self.encoder(x)
            ...
            if self.loss.norm_to_0_1:
                loss_im = self.post_process(self.img_tmp) / 255.0
                loss_rec = self.post_process(self.rec_tmp) / 255.0
            else:
                loss_im = self.img_tmp
