...
def pixel_unshuffle(input, out_size, chunked=False):
    ...
    output = torch.nn.functional.fold(output, output_size=out_size, kernel_size=(1, 1),
                                      stride=(1, 1))  # B x kh*kw*c x H/r x W/r
...
def pixel_shuffle(input, out_size, chunked=False):
    ...
    output = torch.nn.functional.fold(output, output_size=out_size, kernel_size=(kh, kw),
                                      stride=(kh, kw))  # B x c/r^2, H*r W*r
...
def st_gumbel_softmax(logits, temperature, latent_dim, cate_dim, hard=False, return_ind=False):
    """
    ST-gumple-softmax
    input: [*, n_class]
