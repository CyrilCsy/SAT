...
class VectorQuantizer(nn.Module):
    ...
    def __init__(self,
        ...
        assert n_cluster != 1, 'when num of clusters is 1, set n_cluster = 0'
    ...
    def update_semantic_label(self):
        d = torch.ones(self.n_e, self.n_cluster).to(self.device)
        if self.n_cluster > 0:
            if self.semantic_label is None:
                l_c = torch.arange(end=self.n_cluster)
                l_f = torch.randint(0, self.n_cluster, size=(self.n_e - self.n_cluster, 1)).squeeze()
                self.semantic_label = torch.cat((l_c, l_f), dim=0).to(self.device)
            else:
                # with torch.no_grad:
                d = self.get_distance(self.embed_weight[self.n_cluster:])
                label_ = torch.argmax(0 - d[:, :self.n_cluster], dim=1)
                self.semantic_label[self.n_cluster:] = label_
        else:
            self.semantic_label = torch.ones(self.n_e).to(self.device)
        return d
        # print(self.semantic_label[10:30])
    ...
    def get_index(self, logits, topk=1, step=None, total_steps=None, gumbel_sample=False):
        """
        logits: BHW x N
        ...
        if self.gumbel_sample or gumbel_sample:
            gumbel = True
            if self.training:
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        if self.n_cluster == 0:
            ...
            likelihood_value = d_from_cluster     # 1024 x 16
            pos_emb = self.pos_enc(z).view(-1, self.e_dim)  # 1024 x 512
            likelihood_pos = pos_emb @ self.embed_weight[:self.n_cluster, :].t()
            likelihood_pos = self.semantic_pos(likelihood_pos)
            likelihood = F.gumbel_softmax(likelihood_value + likelihood_pos, dim=1, tau=0.5, hard=False)
            token_semantic_type = likelihood * torch.mean(embed_semantic_label, dim=0)  # P(k|z) = P(z|k) * P(k)
            selector = token_semantic_type @ embed_semantic_label.t()
            d_ = d * selector
            min_encoding_indices = self.get_index(d_, topk=topk, step=step, total_steps=total_steps)

            used_cluster = torch.argmax(token_semantic_type, dim=1).unique()
        ...
        return z_q, min_encoding_indices, cls_loss, orth_loss, used_cluster
    ...
    def forward(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        output = {
            'quantize': z_q,
            'used_quantize_embed': torch.zeros_like(loss) + (unique_idx >= 0).sum(),
            'used_cluster': torch.zeros_like(loss) + (used_cluster >= 0).sum(),
            'quantize_loss': loss,
            'cls_loss': cls_loss,
            'orth_loss': orth_loss,
...
class PatchEncoder2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.unfold(x, kernel_size=(self.stride, self.stride),
                                       stride=(self.stride, self.stride))  # B x 3*patch_size^2 x L
...
class VQConvEncoder2(nn.Module):
    def __init__(self, *,
                 in_ch=3,
                 res_ch=256,
                 out_ch,
...
class ConvEncoder2(nn.Module):
    def __init__(self, in_ch, up_layers=None, with_res_block=True, res_block_bottleneck=2,
                 downsample_layer='downsample',
                 partial_conv=False):
        super().__init__()

        out_channels = []
        if isinstance(up_layers, nn.Sequential):
            for layer in up_layers:
                out_channels.append(layer.out_channels)
        else:
            out_channels = up_layers
...
class VQConvDecoder2(nn.Module):
    def __init__(self, *,
                 in_ch,
                 res_ch,
                 out_ch=3,
        ...
        if self.up_layer_with_image:
            ...
            for l in range(len(self.up_layers)):
                if isinstance(self.up_layers[l], UpSample):
                    ...
                    # if l <= len(self.up_layers):
                    #     x = x * mask_ + x_ * (1 - mask_)
                    # else:
...
"""
get_codebook(): return codebook{'default':{'code':embedding, 'label':default_label}
get_semantic_label(): return semantic_label
get_number_of_tokens(): return n_e
get_features(data, ...): return f before quantized and e after quantized
get_codebook_entry_with_token(indices, ...): return embedding via indices
get_tokens_with_feature(feat, ...): return output['index'] of VectorQuantizer
get_tokens(data, ...): get_features(data, ...) + get_tokens_with_feature(feat, ...)
"""
class PatchVQGAN(BaseCodec):
    ...
    def get_semantic_label(self):
        return self.quantize.get_semantic_label()

    def pre_process(self, data):
        data = data.to(self.device)
        # data = data / 127.5 - 1.0
        data = (data / self.im_process_info['scale'].to(data.device) - self.im_process_info['mean'].to(data.device)) / \
               self.im_process_info['std'].to(data.device)
        return data

    def multi_pixels_with_mask(self, data, mask):
        if self.im_process_info['mean'].sum() != 0.0:
            eps = 1e-3
            if data.max() > (((255.0 / self.im_process_info['scale'] - self.im_process_info['mean']) /
                              self.im_process_info['std']).max().to(data.device) + eps):
            ...
            data_m[~mask] = ((torch.zeros_like(data_m) - self.im_process_info['mean'].to(data_m.device)) /
                             self.im_process_info['std'].to(data_m.device))[~mask]
        else:
    ...
    def post_process(self, data):
        # data = (data + 1.0) * 127.5
        data = (data * self.im_process_info['std'].to(data.device) + self.im_process_info['mean'].to(data.device)) * \
               self.im_process_info['scale'].to(data.device)
    ...
    def sample(self, batch):
        ...
        if 'erase_mask' in batch:
            token_type_erase = get_token_type(batch['erase_mask'], token_shape, type=self.resize_mask_type).to(
                self.device)  # B x 1 x H x W
        else:
            token_type_erase = torch.ones((x.shape[0], 1, x.shape[2], x.shape[3])).long().to(self.device)
    ...
    def forward(self, batch, name='none', return_loss=True, step=0, total_steps=None, **kwargs):
        ...
        if name == 'generator':
            ...
            # if 'erase_mask' in batch:
            #     token_type_erase = get_token_type(batch['erase_mask'], token_shape,
            #                                       type=self.resize_mask_type)  # B x 1 x H x W
            # else:
            #     token_type_erase = torch.ones((x.shape[0], 1, x.shape[2], x.shape[3])).long().to(self.device)

            quant_out = self.quantize(x, token_type=None, step=step, total_steps=total_steps)
