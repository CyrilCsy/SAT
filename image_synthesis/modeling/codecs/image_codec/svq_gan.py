...
class VectorQuantizer(nn.Module):
    ...
    def update_semantic_label(self):
        ...
        if self.n_cluster > 0:
            if self.semantic_label is None:
                ...
                # with torch.no_grad:
                d = self.get_distance(self.embedding[self.n_cluster:])
    ...
    def get_distance(self, z, code_type='all'):
        ...
        if self.distance_type == 'euclidean':
            ...
            norm = torch.sqrt(
                torch.sum(z ** 2, dim=1).unsqueeze(dim=1) * torch.sum(self.embedding ** 2, dim=1).unsqueeze(dim=0))
            d = 0 - (0.5 + 0.5 * d / norm)
        else:
    ...
    def orth_loss(self, lamb=1e-3):
        w = self.embedding[:self.n_cluster]
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        if self.n_cluster == 0:
            ...
            sem_cls = sem_cls.expand(sem_cls[0], sem_cls[1], b).view(self.n_cluster, -1).permute(1, 0)
