...
class ImageListDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 zero_mask=0.0, # the probability to set the mask all zeros  # reason ?
                 provided_mask_name='',
                 ...
                 erase_image_with_mask=0.0,
                 erase_image_with_mask_ratio=[0.0, 0.1],
        ...
        my_print('Number of {}: {}'.format(self.name, len(self.data)))
    ...
    def get_mask(self, im_size, index=None):
        # if erase_mask:
        #     if self.stroken_mask_params is None:
        #         stroken_mask_params = {
        #             'max_parts': 15,
        ...
        if self.masks is not None and random.random() < self.use_provided_mask:
            ...
            if self.stroken_mask_params is None:
                ...
                stroken_mask_params = self.stroken_mask_params
            stroken_mask_params['im_size'] = im_size
            stroken_mask_params['keep_ratio'] = [1.0 - self.use_provided_mask_ratio[1], 1.0 - self.use_provided_mask_ratio[0]]
            mask = generate_stroke_mask(**stroken_mask_params)
        if len(mask.shape) == 3:
            mask = mask[:, :, 0:1]
